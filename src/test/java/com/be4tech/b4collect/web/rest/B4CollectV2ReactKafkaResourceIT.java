...
import com.be4tech.b4collect.config.KafkaSseConsumer;
import com.be4tech.b4collect.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class B4CollectV2ReactKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/b-4-collect-v-2-react-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/b-4-collect-v-2-react-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/b-4-collect-v-2-react-kafka/unregister"));
                return;
            }
        }
